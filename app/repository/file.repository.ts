 ...
 saveFile = async (username: string, file: File, serviceKey: string) => {
  const result = await this.provider.query(
   ...
   [username, serviceKey, file.content_type, file.title, file.description,
    file.external_file_id, file.created_time, file.last_update_time, file.is_public]);
  return result.rows[0].id;
 }

 getFile = async (username: string, id: number): Promise<File> => {
  let res = await this.provider.query('SELECT * FROM files WHERE (username = $1 or is_public = true) and id = $2',
   [username, id]);
  return res.rows[0];
 }
}
