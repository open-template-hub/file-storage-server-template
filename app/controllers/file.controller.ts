...
export class FileController {
  createFile = async (context: Context, file: File): Promise<any> => {
    ...
    const serviceClient = await this.getServiceClient(context.mongoDbProvider, context.serviceKey);
    ...
    if (file.uploaded) {
      ...
      return await fileRepository.saveFile(context.username, file, context.serviceKey);
    }
  ...
  downloadFile = async (context: Context, id: any): Promise<File> => {
    ...
    const serviceClient = await this.getServiceClient(context.mongoDbProvider, file.service_key);
  ...
  private isValidFile = (file: File): boolean => {
    if (file.title && file.content_type && file.data && file.description && file) {
      return true;
    }
    return false;
  }
